...
const AS = '/assets/';
const VERSION = 5;
const VERSION_LABEL = 'Website Version 5 · July 2026';
...
let ui = { menu:null, burger:false, heroIndex:0, view:'grid', studioMode:'edit', adminTab:'content', adminPreviewRoute:'home', adminPreviewHistory:['home'], aiOpen:false, aiPanel:'search', toast:'', lastRich:null, autoScroll:null, log:'WOM v5 ready. Admin preview navigates internally and publishes to the public localhost view.' };
...
function renderBottomNav(r){
  const items=[['home','🏠','Home'],['explore','🔍','Explore'],['post','＋','Post'],['saved','♡','Saved'],['profile','👤','Profile']];
  return `<nav class="bottom-nav">${items.map(([rr,ico,label])=>`<button class="${r===rr?'active':''}" data-route="${rr}"><span class="ico">${ico}</span>${label}</button>`).join('')}</nav>`;
}
...
function bind(){
  ...
  bindSwipe(); bindButtons(); bindForms(); bindAI(); if(route==='-admin') bindAdmin();
}
